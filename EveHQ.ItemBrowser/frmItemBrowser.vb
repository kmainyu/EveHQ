...
Public Class frmItemBrowser
    ...
    Public Sub GenerateSkills(ByVal typeID As Long, ByVal typeName As String)
        ...
        For skillNo As Integer = 1 To 3
            If itemSkills(skillNo, 0) <> 0 Then
                If itemSkills(skillNo, 1) <> "" Then
                    skillsRequired = True
                    Dim skillID As String = itemSkills(skillNo, 0)
                    ...
                    Do Until level = 0
                        ' Start @ root!
                        cSkill = EveHQ.Core.HQ.SkillListID(CStr(curSkill))

                        ' Read pointer @ level
                        Select Case pointer(level)
                            Case 1
                                If cSkill.PS = curSkill Then Exit Do
                                pointer(level) = 2
                                curSkill = cSkill.PS
                                curLevel = cSkill.PSL
                            Case 2
                                If cSkill.SS = curSkill Then Exit Do
                                pointer(level) = 3
                                curSkill = cSkill.SS
                                curLevel = cSkill.SSL
                            Case 3
                                If cSkill.TS = curSkill Then Exit Do
                                pointer(level) = 4
                                curSkill = cSkill.TS
                                curLevel = cSkill.TSL
                            Case 4
