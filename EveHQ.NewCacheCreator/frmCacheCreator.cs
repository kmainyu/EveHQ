...
namespace EveHQ.NewCacheCreator
{
    ...
    public partial class FrmCacheCreator
    {
        ...
        static DataSet marketGroupData;
        static DataSet shipGroupData;
        static DataSet shipNameData;
        static DataSet moduleData;
        static DataSet moduleEffectData;
        static DataSet moduleAttributeData;
        ...
        // Key = CertID
        static SortedList<int, YAMLCert> yamlCerts = new SortedList<int, YAMLCert>();
                ...
                using (var reader = new StreamReader(dataStream))
                {
                    ...
                    if (yaml.Documents.Any())
                    {
                        ...
                        foreach (var entry_loopVariable in root.Children)
                        {
                            ...
                            foreach (var subEntry_loopVariable in ((YamlMappingNode)entry.Value).Children)
                            {
                                        ...
                                        case "skillTypes":
                                            ...
                                            if (skillMaps != null)
                                            {
                                                foreach (var skillMap_loopVariable in skillMaps.Children)
                                                {
                                                    ...
                                                    foreach (var level_loopVariable in ((YamlMappingNode)skillMap.Value).Children)
                                                    {
                ...
                using (var reader = new StreamReader(dataStream))
                {
                    ...
                    if (yaml.Documents.Any())
                    {
                        ...
                        foreach (var entry_loopVariable in root.Children)
                        {
                            ...
                            YAMLType yamlItem = new YAMLType();
                                    ...
                                    switch (keyName)
                                    {
                                        ...
                                        case "traits":
                                            ...
                                            foreach (var skill_loopVariable in traits.Children)
                                            {
                                                ...
                                                foreach (var index_loopVariable in ((YamlMappingNode)skill.Value).Children)
                                                {
                                                    var index = index_loopVariable;
                                                    string partBonus = "";
                                                    string partBonusText = "";
                                                    string partUnitID = "";
                                                    foreach (var bonusPart_loopVariable in ((YamlMappingNode)index.Value).Children)
                                                    {
                                                        var bonusPart = bonusPart_loopVariable;
        ...
        private void LoadMasteries()
        {
            ...
            foreach (var type_loopVariable in yamlTypes.Values.Where(i => i.Masteries != null))
            {
                var type = type_loopVariable;
                Mastery mastery = new Mastery();
                mastery.TypeId = type.TypeID;
                mastery.RequiredCertificates = new SortedList<int, List<int>>();
                foreach (var rank_loopVariable in type.Masteries)
                {
                    var rank = rank_loopVariable;
                    mastery.RequiredCertificates.Add(rank.Key, new List<int>());
                    foreach (var cert_loopVariable in rank.Value)
                    {
                        var cert = cert_loopVariable;
        ...
        private void LoadTraits()
        {
            ...
            foreach (var type_loopVariable in yamlTypes.Values.Where(i => i.Traits != null))
            {
                var type = type_loopVariable;
        ...
        private void LoadCertCategories()
        {
            ...
            foreach (var groupID_loopVariable in yamlCerts.Values.Select(c => c.GroupID).Distinct())
            {
                var groupID = groupID_loopVariable;
                CertificateCategory newCat = new CertificateCategory();
                newCat.Id = groupID;
        ...
        private void LoadCerts()
        {
            ...
            foreach (var cert_loopVariable in yamlCerts.Values)
            {
                var cert = cert_loopVariable;
                ...
                foreach (var reqSkill_loopVariable in cert.RequiredSkills)
                {
                    var reqSkill = reqSkill_loopVariable;
            ...
            foreach (var cert_loopVariable in StaticData.Certificates.Values)
            {
                var cert = cert_loopVariable;
                foreach (var gradeAndSkills_loopVariable in cert.GradesAndSkills)
                {
                    var gradeAndSkills = gradeAndSkills_loopVariable;
        ...
        private void LoadCertRecs()
        {
            ...
            foreach (var certRow_loopVariable in yamlCerts.Values)
            {
                var certRow = certRow_loopVariable;
                foreach (var shipRec_loopVariable in certRow.RecommendedFor)
                {
                    var shipRec = shipRec_loopVariable;
                    CertificateRecommendation certRec = new CertificateRecommendation();
        ...
        private void LoadUnlocks()
        {
            ...
            using (DataSet evehqData = GetStaticData(strSQL))
            {
                for (int row = 0; row <= evehqData.Tables[0].Rows.Count - 1; row++)
                {
                    ...
                    {
                        DataRow[] attRows = evehqData.Tables[0].Select("invTypeID=" + evehqData.Tables[0].Rows[row]["invtypeID"].ToString());
                        const int MaxPreReqs = 10;
                        string[] preReqSkills = new string[MaxPreReqs + 1];
                        int[] preReqSkillLevels = new int[MaxPreReqs + 1];
                        ...
                        for (int prereq = 1; prereq <= MaxPreReqs; prereq++)
                        {
                            if (!string.IsNullOrEmpty(preReqSkills[prereq]))
                            {
                                ...
                                itemList.Add(skillIDLevel + "_" + evehqData.Tables[0].Rows[row]["invtypeID"].ToString() + "_" + evehqData.Tables[0].Rows[row]["groupID"].ToString());
                            }
                        }
                ...
                foreach (Certificate cert in StaticData.Certificates.Values)
                {
                    ...
                    {
                        string skillID = skill + "." + cert.GradesAndSkills[CertificateGrade.Basic][skill].ToString();
        ...
        private void LoadSolarSystems()
        {
            ...
            using (SqlConnection connection = new SqlConnection(StaticDBConnection))
            {
                ...
                int id = 0;
                SqlCommand command = new SqlCommand("SELECT * FROM mapDenormalize;", connection);
                connection.Open();
        ...
        private void LoadTypeMaterials()
        {
            ...
            using (DataSet evehqData = GetStaticData("SELECT * FROM invTypeMaterials;"))
            {
                for (int item = 0; item <= evehqData.Tables[0].Rows.Count - 1; item++)
                {
                    ...
                    if (StaticData.TypeMaterials.ContainsKey(typeID) == true)
                    {
        ...
        private void LoadBlueprints()
        {
            ...
            foreach (Blueprint bp in StaticData.Blueprints.Values)
            {
                ...
                DataRow[] bpRows = evehqData.Tables[0].Select("typeID=" + bp.Id.ToString());
                foreach (DataRow req in bpRows)
                {
                    BlueprintResource newReq = new BlueprintResource();
        ...
        private void LoadAssemblyArrays()
        {
            ...
            {
                AssemblyArray newArray = new AssemblyArray();
        ...
        {
            string[] attributeLines = ResourceHandler.GetResource("Attributes").ToString().Split(ControlChars.CrLf.ToCharArray());
            string[] att = null;
            Attribute attData = default(Attribute);
        ...
        private void LoadEveSkillData()
        {
            ...
            List<int> skillAttFilter = new List<int>();
            ...
            foreach (int groupID in groupIDs)
            {
                if (groupID != 267)
                {
                    Core.SkillGroup newSkillGroup = new Core.SkillGroup();
                    ...
                    {
                        Core.EveSkill newSkill = new Core.EveSkill();
                        newSkill.ID = item.Id;
            ...
            {
                int[] preReqSkills = new int[MaxPreReqs + 1];
                int[] preReqSkillLevels = new int[MaxPreReqs + 1];
                ...
                if (shipData != null)
                {
                    if (shipData.Tables[0].Rows.Count != 0)
                    {
                        ...
                        foreach (DataRow shipRow in shipData.Tables[0].Rows)
                        {
                            ...
                            if (Information.IsDBNull(shipRow["valueInt"]) == true)
                            {
                            ...
                            switch (Convert.ToInt32(shipRow["attributeID"]))
                            {
                              ...
                              EveType tSkill = StaticData.Types[Convert.ToInt32(attValue)];
                                    ...
                                    break;
                                case 277:
                                    if (newShip.RequiredSkills.ContainsKey(pSkillName) == true)
                                    {
                                    ...
                                    break;
                                case 278:
                                    if (newShip.RequiredSkills.ContainsKey(sSkillName) == true)
                                    {
                                    ...
                                    break;
                                case 279:
                                    if (newShip.RequiredSkills.ContainsKey(tSkillName) == true)
                                    {
        ...
        private void BuildModuleData()
        {
            try
            {
                ...
                for (int setNo = 0; setNo <= 1; setNo++)
                {
                    foreach (DataRow row in moduleData.Tables[0].Rows)
                    {
                        if (Information.IsDBNull(row["marketGroupID"]) == true)
                        {
                            modID = Convert.ToInt32(row["typeID"]);
                            nModule = ModuleLists.ModuleList[modID];
                            if (ModuleLists.ModuleMetaTypes.ContainsKey(modID) == true)
                            {
                ...
                string[] changeLines = ResourceHandler.GetResource("newItemMarketGroup").ToString().Split(ControlChars.CrLf.ToCharArray());
                foreach (string marketChange in changeLines)
                {
                    if (!string.IsNullOrEmpty(marketChange.Trim()))
                    {
                        ...
                        if (ModuleLists.ModuleList.ContainsKey(typeID) == true)
                        {
            ...
            try
            {
                ...
                foreach (DataRow modRow in moduleEffectData.Tables[0].Rows)
                {
                    ...
                    if (effMod != null)
                    {
                        ...
                        if (effMod.IsDrone == true | effMod.IsLauncher == true | effMod.IsTurret == true | effMod.DatabaseGroup == 72 | effMod.DatabaseGroup == 862)
                        {
                ...
                if (BuildModuleAttributeData() == true)
                {
                    return;
                }
                else
        ...
        private bool BuildModuleAttributeData()
        {
            try
            {
                ...
                foreach (DataRow modRow in moduleAttributeData.Tables[0].Rows)
                {
                    ...
                    switch (Convert.ToInt32(modRow["attributeID"]))
                    {
                        ...
                        case 6:
                            ...
                            break;
                        case 51:
                            if (attMod.Attributes.ContainsKey(6) == true)
                            {
                        ...
                        case 1087:
                            ...
                            break;
                        case 182:
                            if (StaticData.Types.ContainsKey(Convert.ToInt32(attValue)) == true)
                            {
                                EveType pSkill = StaticData.Types[Convert.ToInt32(attValue)];
                                ItemSkills nSkill = new ItemSkills();
                                nSkill.ID = pSkill.Id;
                                nSkill.Name = pSkill.Name;
                                pSkillName = pSkill.Name;
                ...
                {
                    if (ModuleLists.ModuleMetaGroups.ContainsKey(cMod.ID) == true)
                    {
                        if (ModuleLists.ModuleMetaGroups[cMod.ID] == 0)
                        {
                            if (cMod.Attributes.ContainsKey(422) == true)
                            {
                ...
                foreach (ShipModule cMod in ModuleLists.ModuleList.Values)
                {
                    if (cMod.IsDrone == true & cMod.Attributes.ContainsKey(507) == true)
                    {
        ...
        private bool BuildImplantData()
        {
            try
            {
                ...
                {
                    if (impMod.IsImplant == true)
                    {
                        Implants.ImplantList.Add(impMod.Name, impMod);
                    }
                    if (impMod.IsBooster == true)
                    {
                ...
                foreach (string cImplant in implantsList)
                {
                    if (!string.IsNullOrEmpty(cImplant.Trim()) & cImplant.StartsWith("#", StringComparison.Ordinal) == false)
                    {
                        ...
                        if (Implants.ImplantList.ContainsKey(implantName) == true)
                        {
        ...
        private void BuildModuleEffects()
        {
            ...
            List<string> effectLines = ResourceHandler.GetResource("Effects").ToString().Split(ControlChars.CrLf.ToCharArray()).ToList();
            ...
            foreach (string effectLine in effectLines)
            {
                if (!string.IsNullOrEmpty(effectLine.Trim()) & effectLine.StartsWith("#", StringComparison.Ordinal) == false)
                {
                    ...
                    foreach (string affectingID in affectingIDs)
                    {
                        ...
                        HQFEffectType temp;
                        newEffect.AffectingType = HQFEffectType.TryParse(effectData[1], out temp) ? temp : 0;
                        newEffect.AffectingID = Convert.ToInt32(affectingID);
                        newEffect.AffectedAtt = Convert.ToInt32(effectData[3]);
                        newEffect.AffectedType = HQFEffectType.TryParse(effectData[4], out temp) ? temp : 0;
                        if (effectData[5].Contains(";") == true)
                        {
                        ...
                        newEffect.StackNerf = EffectStackType.TryParse(effectData[6], out tempStack) ? tempStack : 0;
                        newEffect.IsPerLevel = Convert.ToBoolean(effectData[7]);
                        newEffect.CalcType = EffectCalcType.TryParse(effectData[8], out tempCalc) ? tempCalc : 0;
                        ...
                        switch (newEffect.AffectingType)
                        {
                            ...
                            case HQFEffectType.Item:
                                if (newEffect.AffectingID > 0)
                                {
                                    ...
                                    if (Core.HQ.SkillListName.ContainsKey(affectingName) == true)
                                    {
                        ...
                        {
                            if (affectingName.Contains(";Skill;") == true)
                            {
        ...
        {
            // Break the Effects down into separate lines
            string[] effectLines = ResourceHandler.GetResource("ImplantEffects").ToString().Split(ControlChars.CrLf.ToCharArray());
            // Go through lines and break each one down
            ...
            foreach (string effectLine in effectLines)
            {
                if (!string.IsNullOrEmpty(effectLine.Trim()) & effectLine.StartsWith("#", StringComparison.Ordinal) == false)
                {
                    ...
                    foreach (string att in atts)
                    {
                        ...
                        affectingName = StaticData.Types[Convert.ToInt32(effectData[2])].Name + ";Implant;" + Attributes.AttributeQuickList[newEffect.AffectedAtt].ToString() + ";";
        ...
        private void BuildShipEffects()
        {
            ...
            // Break the Effects down into separate lines
            string[] effectLines = ResourceHandler.GetResource("ShipBonuses").ToString().Split(ControlChars.CrLf.ToCharArray());
            // Go through lines and break each one down
            string[] effectData = null;

            ArrayList shipEffectClassList = new ArrayList();
        ...
        private void BuildSubsystemEffects()
        {
            ...
            // Break the Effects down into separate lines
            string[] effectLines = ResourceHandler.GetResource("Subsystems").ToString().Split(ControlChars.CrLf.ToCharArray());
            // Go through lines and break each one down
            string[] effectData = null;
        ...
        {
            TreeView tvwShips = new TreeView();
            ...
            {
                TreeNode rootNode = new TreeNode(Convert.ToString(rootRow["marketGroupName"]));
            ...
            {
                shipGroup = factionRow["groupName"].ToString() + "s";
        ...
        {
            TreeView tvwItems = new TreeView();
            ...
            TreeNode favNode = new TreeNode("Favourites");
            favNode.Name = "Favourites";
            favNode.Tag = "Favourites";
            tvwItems.Nodes.Add(favNode);
            // Add the Favourties Node
            TreeNode mruNode = new TreeNode("Recently Used");
        ...
        {
            StreamWriter sw = new StreamWriter(Path.Combine(coreCacheFolder, "ShipGroups.bin"));
        ...
        {
            StreamWriter sw = new StreamWriter(Path.Combine(coreCacheFolder, "ItemGroups.bin"));
            foreach (TreeNode rootNode in tvwItems.Nodes)
            {
        ...
        private void AddSQLAttributeGroupColumn(SqlConnection connection)
        {
            ...
            SqlCommand keyCommand = new SqlCommand(strSQL, connection);
            ...
            foreach (string line_loopVariable in lines)
            {
                ...
                if (line.StartsWith("attributeID", StringComparison.Ordinal) == false & !string.IsNullOrEmpty(line))
                {
                    ...
                    SqlCommand keyCommand2 = new SqlCommand(strSQL2, connection);
        ...
        {
            string strSQL = Resources.SQLQueries.ToString();
            SqlCommand keyCommand = new SqlCommand(strSQL, connection);
            keyCommand.ExecuteNonQuery();
        ...
        private void btnCheckMarketGroup_Click(object sender, EventArgs e)
        {
            ...
            List<int> marketGroups = new List<int>();
