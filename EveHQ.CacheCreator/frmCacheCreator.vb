...
Public Class FrmCacheCreator
    ...
    Private Const BulkDBConnection As String = "Server=localhost\SQLEXPRESS; Database = " & BulkDB & "; Integrated Security = SSPI;" ' For SQL DBE connection
    ...
    ReadOnly _sqLiteDbcs As String = "Data Source=" & ControlChars.Quote & _sqLiteDB & ControlChars.Quote & ";Version=3;" ' For SQLite DB connection
    ...
    ' Eve Cache variables
    ReadOnly _primaryKeys As New Dictionary(Of String, String)
    ...
    Private Sub frmCacheCreator_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        End If
        DefinePrimaryKeys()
        lblDB.Text = "Database: " & StaticDB
        Call UpdateEveLocationInfo()
    End Sub
...
#Region "HQF Icon Generation Routines"

    Private Sub btnGenerateHQFIcons_Click(sender As Object, e As EventArgs) Handles btnGenerateHQFIcons.Click
        Call GenerateHQFIcons()
    End Sub

    ''' <summary>
    ''' Looks at the list of modules, identifies the icon names and copies
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub GenerateHQFIcons()

        ' Declare paths
        Dim basePath As String = Path.Combine(Application.StartupPath, "HQFIconCache")
        Dim iconPath As String = Path.Combine(basePath, "items")
        Dim cachePath As String = Path.Combine(basePath, "cache")

        ' Declare default icon size
        Const defaultIconSize As Integer = 64

        ' Declare a definitive list of icons
        Dim iconList As New List(Of String)

        ' Cycle through each module, but declare some stuff first
        Dim iconData() As String
        Dim iconGroup As Integer
        Dim iconType As Integer
        Dim successCount As Integer = 0
        Dim droneCount As Integer = 0
        Dim failedMods As New List(Of String)
        For Each sMod As ShipModule In ModuleLists.moduleList.Values
            Dim iconName As String
            Dim sourceFile As String
            Dim destFile As String
            ' First, check if the icon actually exists in the pack

            If sMod.Icon IsNot Nothing Then
                iconName = sMod.Icon.TrimEnd(".png".ToCharArray)

                ' Check if we have the icon file already
                If iconList.Contains(iconName) = False Then
                    sourceFile = Path.Combine(iconPath, iconName & ".png")
                    destFile = Path.Combine(cachePath, "_" & iconName & ".png")
                    If My.Computer.FileSystem.FileExists(sourceFile) = True Then
                        Try
                            If My.Computer.FileSystem.FileExists(destFile) = True Then
                                My.Computer.FileSystem.DeleteFile(destFile)
                            End If
                            My.Computer.FileSystem.CopyFile(sourceFile, destFile, True)
                        Catch e As Exception
                            ' TODO: File already exists - not sure why we can't delete it tho!
                        End Try
                        successCount += 1
                        iconList.Add(iconName)
                        sMod.Icon = iconName
                    Else
                        ' Check for an underscore in the icon
                        iconData = iconName.Split("_".ToCharArray)
                        If iconData.Length = 2 Then
                            iconGroup = CInt(iconData(0))
                            iconType = CInt(iconData(1))
                            ' Build the path to the true icon
                            sourceFile = Path.Combine(iconPath, iconGroup.ToString & "_" & defaultIconSize.ToString & "_" & iconType.ToString & ".png")
                            destFile = Path.Combine(cachePath, "_" & iconName & ".png")
                            If My.Computer.FileSystem.FileExists(sourceFile) = False Then
                                sourceFile = Path.Combine(iconPath, iconGroup.ToString & "_" & (defaultIconSize / 2).ToString & "_" & iconType.ToString & ".png")
                            End If
        ...
        msg.AppendLine("Module Count: " & ModuleLists.moduleList.Count.ToString("N0"))
        msg.AppendLine("Success Count: " & successCount.ToString("N0"))
        msg.AppendLine("Failure Count: " & failedMods.Count.ToString("N0"))
        msg.AppendLine("Drone Count: " & droneCount.ToString("N0"))
        MessageBox.Show(msg.ToString, "Conversion Complete", MessageBoxButtons.OK, MessageBoxIcon.Information)
...
#Region "Eve Cache Routines"

    Private Sub DefinePrimaryKeys()

        _primaryKeys.Clear()
        For Each key As String In My.Resources.PKs.ToString.Split(ControlChars.CrLf.ToCharArray)
            If key <> "" Then
                Dim kv() As String = key.Split(",".ToCharArray)
                _primaryKeys.Add(kv(0), kv(1))
            End If
        Next

    End Sub

    Private Sub UpdateEveLocationInfo()
        btnEveLocation.Text = _eveLocation
        lblEveProtocol.Text = EveCacheParserFunctions.GetEveCacheProtocol(_eveLocation, _eveServer).ToString
    End Sub

    Private Sub btnEveLocation_Click(sender As Object, e As EventArgs) Handles btnEveLocation.Click
        Dim fbd As New FolderBrowserDialog
        With fbd
            .Description = "Please select the folder where the Eve executable is located..."
            .ShowNewFolderButton = False
            .SelectedPath = _eveLocation
            .ShowDialog()
            If My.Computer.FileSystem.FileExists(Path.Combine(.SelectedPath, "eve.exe")) = False Then
                MessageBox.Show("This folder does not contain the Eve.exe file.", "Eve Executable Not Found", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                _eveLocation = .SelectedPath
                Call UpdateEveLocationInfo()
            End If
        End With
        fbd.Dispose()
    ...
    Private Sub rbTranquility_CheckedChanged(sender As Object, e As EventArgs) Handles rbTranquility.CheckedChanged
        _eveServer = EveServerName.Tranquility
        UpdateEveLocationInfo()
    End Sub

    Private Sub rbSingularity_CheckedChanged(sender As Object, e As EventArgs) Handles rbSingularity.CheckedChanged
        _eveServer = EveServerName.Singularity
        UpdateEveLocationInfo()
    End Sub

    Private Sub rbDuality_CheckedChanged(sender As Object, e As EventArgs) Handles rbDuality.CheckedChanged
        _eveServer = EveServerName.Duality
        UpdateEveLocationInfo()
    End Sub

    Private Sub rbBuckingham_CheckedChanged(sender As Object, e As EventArgs) Handles rbBuckingham.CheckedChanged
        _eveServer = EveServerName.Buckingham
        UpdateEveLocationInfo()
    End Sub
    ...
    Private Sub ExportDataAsXML()
        ' First, let's export as much bulk data as possible
        Call ExportData(DumpMode.XML, True)
        ' Next, export stuff in the CachedMethods and CachedObjects folders in the machonet
        Call ExportData(DumpMode.XML, False)
    End Sub

    Private Sub ExportDataAsSQL()
        ' First, let's export as much bulk data as possible
        Call ExportData(DumpMode.SQL, True)
        ' Next, export stuff in the CachedMethods and CachedObjects folders in the machonet
        Call ExportData(DumpMode.SQL, False)
    End Sub

    Private Sub ExportData(mode As DumpMode, isBulkExport As Boolean)

        Dim count As Integer = 0

        Dim startTime As DateTime, endTime As DateTime

        WriteLogEntry("Enumerating required files")
        startTime = DateTime.Now

        Parser.SetCachedFilesFolders("CachedMethodCalls")
        Parser.SetCachedFilesFolders("CachedObjects")

        Dim cachedFiles As IEnumerable(Of FileInfo)
        If isBulkExport = True Then
            cachedFiles = Parser.GetBulkDataCachedFiles(_eveLocation)
        Else
            'Dim cacheLocation As String = EveCacheParserFunctions.GetEveMarketCacheLocation(_eveLocation, _eveServer)
            cachedFiles = EveCacheParserFunctions.GetEveCacheFiles(_eveLocation, _eveServer)
        End If

        endTime = DateTime.Now
        WriteLogEntry("Time taken to enumerate = " & (endTime - startTime).TotalMilliseconds & "ms")

        ' Enumerate all files
        WriteLogEntry("Found " & cachedFiles.Count.ToString & " files...")
        For Each cachedFile As FileInfo In cachedFiles

            count += 1

            ' Get cache file type from the constants
            Dim cacheTypeNumber As Integer
            Dim cacheTypeString As String
            If isBulkExport = True Then
                cacheTypeNumber = CType(CInt(Path.GetFileNameWithoutExtension(cachedFile.Name)), CacheTypeConstants)
                cacheTypeString = [Enum].GetName(GetType(CacheTypeConstants), cacheTypeNumber)
            Else
                cacheTypeNumber = CInt("&H" & Path.GetFileNameWithoutExtension(cachedFile.Name))
                cacheTypeString = cacheTypeNumber.ToString
            End If

            WriteLogEntry("Parsing file " & count.ToString & " of " & cachedFiles.Count.ToString & " (" & cachedFile.Name & ")..." & cacheTypeString)
            Try
                startTime = DateTime.Now
                Dim result As KeyValuePair(Of Object, Object) = Parser.Parse(cachedFile)

                If isBulkExport = True Then
                    ' This is for a bulk export file
                    If TypeOf result.Value Is List(Of Object) Then
                        If CType(result.Value, List(Of Object)).Count > 0 Then
                            WriteLogEntry("Found " & CType(result.Value, List(Of Object)).Count.ToString & " records!")
                            Select Case mode
                                Case DumpMode.XML
                                    Call ExportBulkDataAsXML(cacheTypeString, CType(result.Value, List(Of Object)))
                                Case DumpMode.SQL
                                    Call ExportBulkDataAsSQL(_sqLiteDbcs, cacheTypeString, CType(result.Value, List(Of Object)))
                            End Select
                        Else
                            WriteLogEntry("No suitable records found!")
                        End If
                    Else
                        WriteLogEntry("Unable to determine result value type!")
                    End If
                Else
                    Call ExportMachoData(mode, result)
                End If

                endTime = DateTime.Now

                WriteLogEntry("Parsed file " & count.ToString & " of " & cachedFiles.Count.ToString & " in " & (endTime - startTime).TotalMilliseconds & "ms")

            Catch ex As ParserException
                WriteLogEntry("Parsing failed: " & ex.Message)
            End Try
        Next

    End Sub

    Private Shared Sub ExportBulkDataAsXML(cacheTypeString As String, result As IEnumerable(Of Object))

        ' Get the cache type as a string
        'Dim cacheTypeString As String = [Enum].GetName(GetType(CacheTypeConstants), cacheType)
        Dim baseName As String = cacheTypeString.TrimStart("cache".ToCharArray)

        ' Try and create a XML folder in the application directory
        Dim xmlFolder As String = Path.Combine(Application.StartupPath, "EveCacheXMLDump")
        Try
            If My.Computer.FileSystem.DirectoryExists(xmlFolder) = False Then
                My.Computer.FileSystem.CreateDirectory(xmlFolder)
            End If
        Catch ex As Exception
            MessageBox.Show("Unable to create XML Dump folder: " & ex.Message, "Error Creating XML Dump", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End Try

        ' Create a new XMLDocument for the dump
        ...
        Dim xmlRoot As XmlElement = xml.CreateElement(cacheTypeString)
        ...
        For Each row As Object In result
            ...
            For Each dataValues As Dictionary(Of Object, Object) In CType(CType(row, Tuple(Of Object)).Item1, List(Of Object))

                ' Create the XML "record"
                Dim xmlr As XmlElement = xml.CreateElement("record")
                xmlRoot.AppendChild(xmlr)

                ' Cycle through the keys
                For Each key As String In dataValues.Keys

                    ' Add the key/value to the XML "record"
                    Dim xmle As XmlElement = xml.CreateElement(key)
                    xmle.InnerText = CStr(dataValues(key))
                    xmlr.AppendChild(xmle)

                Next
            Next
        Next

        ' Export the XMLDocument to disk
        Dim fileName As String = Path.Combine(xmlFolder, baseName & ".xml")
        Try
            xml.Save(fileName)
        Catch ex As Exception
            MessageBox.Show("Unable to save XML file for " & cacheTypeString & ": " & ex.Message, "Unable to create XML Dump file", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try

    End Sub

    Private Shared Sub ExportBulkDataAsSQL(sqlConnection As String, cacheTypeString As String, result As List(Of Object))
        Select Case cacheTypeString
            Case "cacheDogmaTypeAttributes"
                CacheConversionClasses.TypeAttributes.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheDogmaAttributes"
                CacheConversionClasses.AttributeTypes.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheDogmaTypeEffects"
                CacheConversionClasses.TypeEffects.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheDogmaEffects"
                CacheConversionClasses.Effects.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheDogmaUnits"
                CacheConversionClasses.Units.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheInvCategories"
                CacheConversionClasses.Categories.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheInvGroups"
                CacheConversionClasses.Groups.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheInvTypes"
                CacheConversionClasses.Types.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheInvBlueprintTypes"
                CacheConversionClasses.BlueprintTypes.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheInvMetaGroups"
                CacheConversionClasses.MetaGroups.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheInvMetaTypes"
                CacheConversionClasses.MetaTypes.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheInvTypeMaterials"
                CacheConversionClasses.TypeMaterials.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
            Case "cacheRamTypeRequirements"
                CacheConversionClasses.TypeRequirements.UpdateMasterDB(BulkDBConnection, SQLConnection, result)
        End Select
    End Sub

    ''' <summary>
    ''' Parses the results of cache file decoding and exports it according to the dump mode
    ''' </summary>
    ''' <param name="mode">The DumpMode type which will be exported</param>
    ''' <param name="result">The KeyValuePair result from decoding the market cache file</param>
    ''' <remarks></remarks>
    Private Sub ExportMachoData(mode As DumpMode, result As KeyValuePair(Of Object, Object))

        ' Check if we have anything to start with
        If result.Key Is Nothing OrElse result.Value Is Nothing Then
            Exit Sub
        End If

        Dim id As Object = If(TryCast(result.Key, String), If(TryCast(DirectCast(DirectCast(result.Key, Tuple(Of Object)).Item1, List(Of Object))(0), String), TryCast(DirectCast(DirectCast(DirectCast(DirectCast(result.Key, Tuple(Of Object)).Item1, List(Of Object))(0), Tuple(Of Object)).Item1, List(Of Object))(0), String)))
        If TryCast(result.Value, Dictionary(Of Object, Object)) Is Nothing Then
            Exit Sub
        End If

        Dim method As Object = TryCast(DirectCast(DirectCast(result.Key, Tuple(Of Object)).Item1, List(Of Object))(1), String)

        Select Case DirectCast(id, String)
            Case "account"
                ' Do nothing

            Case "agentMgr"

                Select Case DirectCast(method, String)

                    Case "GetAgents"
                        Dim value As List(Of Object) = DirectCast(DirectCast(result.Value, Dictionary(Of Object, Object))("lret"), List(Of Object))
                        Select Case mode
                            Case DumpMode.XML
                                ExportMachoListAsXML(DirectCast(method, String), value)
                            Case DumpMode.SQL
                                'CacheConversionClasses.Agents.UpdateMasterDB(SQLDBCS, SQLiteDBCS, cacheTypeString, result)
                        End Select

                    Case "GetAgentsInSpace"
                        'Dim value As Dictionary(Of Object, Object) = DirectCast(DirectCast(result.Value, Dictionary(Of Object, Object))("lret"), Dictionary(Of Object, Object))
                End Select
            ...
            Case "marketProxy"
                ...
                Select Case DirectCast(method, String)
                    ...
                    Case "GetMarketGroups"
                        ...
                        Select Case mode
                            Case DumpMode.XML
                                ExportMarketGroupsAsXML(DirectCast(method, String), value)
                            Case DumpMode.SQL
                                CacheConversionClasses.MarketGroups.UpdateMasterDB(BulkDBConnection, _sqLiteDbcs, value)
                        End Select
                End Select
            Case "certificateMgr"
                ...
                Select Case DirectCast(method, String)
                    ...
                    Case "GetCertificateCategories"
                        ...
                        Select Case mode
                            Case DumpMode.XML
                                ExportCertificateDictionaryAsXML(DirectCast(method, String), value)
                            Case DumpMode.SQL
                                'CacheConversionClasses.CertificateCategories.UpdateMasterDB(SQLDBCS, SQLiteDBCS, cacheTypeString, result)
                        End Select
                    ...
                    Case "GetCertificateClasses"
                        ...
                        Select Case mode
                            Case DumpMode.XML
                                ExportCertificateDictionaryAsXML(DirectCast(method, String), value)
                            Case DumpMode.SQL
                                'CacheConversionClasses.CertificateClasses.UpdateMasterDB(SQLDBCS, SQLiteDBCS, cacheTypeString, result)
                        End Select

                    Case "GetAllShipCertificateRecommendations"
                        Dim value As List(Of Object) = DirectCast(DirectCast(result.Value, Dictionary(Of Object, Object))("lret"), List(Of Object))
                        Select Case mode
                            Case DumpMode.XML
                                ExportMachoListAsXML(DirectCast(method, String), value)
                            Case DumpMode.SQL
                                'CacheConversionClasses.Recommendations.UpdateMasterDB(SQLDBCS, SQLiteDBCS, cacheTypeString, result)
    ...
    Private Sub WriteLogEntry(logText As String)
        Dim logEntry As New ListViewItem(Now.ToString("yyyy-MM-dd HH:mm:ss:ffff"))
        logEntry.SubItems.Add(logText)
        lvwLog.Items.Add(logEntry)
        lvwLog.Refresh()
    End Sub

    Private Shared Sub ExportMarketGroupsAsXML(method As String, result As Dictionary(Of Object, Object))

        ' Get the cache type as a string
        'Dim cacheTypeString As String = [Enum].GetName(GetType(CacheTypeConstants), cacheType)
        Dim baseName As String = method.TrimStart("Get".ToCharArray)
        ...
        ' Create document root
        Dim xmlRoot As XmlElement = xml.CreateElement(method)
        xml.AppendChild(xmlRoot)
        ...
        For Each row As Object In result.Values
            ...
            ' Get the list of key/values associated with the data
            For Each dataValues As Dictionary(Of Object, Object) In CType(row, List(Of Object))
        ...
        Try
            xml.Save(fileName)
        Catch ex As Exception
            MessageBox.Show("Unable to save XML file for " & method & ": " & ex.Message, "Unable to create XML Dump file", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try

    End Sub

    Private Shared Sub ExportCertificateDictionaryAsXML(method As String, result As Dictionary(Of Object, Object))
    ...
    Private Shared Sub ExportMachoListAsXML(method As String, result As IEnumerable(Of Object))
        ...
        ' Get the list of key/values associated with the data
        For Each dataValues As Dictionary(Of Object, Object) In CType(result, List(Of Object))
...
Friend Enum DumpMode As Integer
    XML = 0
    SQL = 1
End Enum