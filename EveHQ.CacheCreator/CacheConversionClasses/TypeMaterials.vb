...
Namespace CacheConversionClasses
    ...
    Public Class TypeMaterials
        Friend Shared dataCollection As New List(Of TypeMaterial)
        Const tableName As String = "invTypeMaterials"
        Const tableColumns As Integer = 3
        Const deleteSQL As String = "DELETE FROM " & tableName & ";"
        Const createSQL As String = "DROP TABLE IF EXISTS " & tableName & "; CREATE TABLE " & tableName & "(typeID int NOT NULL, materialTypeID int NOT NULL, quantity int NOT NULL);"
        Const insertSQL As String = "INSERT INTO " & tableName & "(typeID, materialTypeID, quantity) VALUES (?,?,?);"
                ...
                Try
                    ...
                    Using sqlCmd As SQLiteCommand = connection.CreateCommand
                        Using sqlTrans As SQLiteTransaction = connection.BeginTransaction
                            ...
                            For Each data As TypeMaterial In dataCollection

                                ' Add the database values and execute the query
                                sqlCmd.Parameters(0).Value = data.typeID
                                sqlCmd.Parameters(1).Value = data.materialTypeID
                                sqlCmd.Parameters(2).Value = data.quantity
    ...
    ''' <summary>
    ''' Class to hold an instance of a TypeMaterial
    ''' </summary>
    ''' <remarks></remarks>
