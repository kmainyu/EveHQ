...
Namespace CacheConversionClasses
    ...
    Public Class TypeAttributes
        ...
        Friend Shared dataCollection As New List(Of TypeAttribute)
        Const tableName As String = "dgmTypeAttributes"
        Const tableColumns As Integer = 4
        Const deleteSQL As String = "DELETE FROM " & tableName & ";"
        Const createSQL As String = "DROP TABLE IF EXISTS " & tableName & "; CREATE TABLE " & tableName & "(typeID int NOT NULL,attributeID int NOT NULL,valueInt int NULL,valueFloat float NULL);"
        Const insertSQL As String = "INSERT INTO " & tableName & "(typeID, attributeID, valueInt, valueFloat) VALUES (?,?,?,?);"
                ...
                Try
                    ...
                    Using sqlCmd As SQLiteCommand = connection.CreateCommand
                        Using sqlTrans As SQLiteTransaction = connection.BeginTransaction
                            ...
                            For Each data As TypeAttribute In dataCollection

                                ' Add the database values and execute the query
                                sqlCmd.Parameters(0).Value = data.typeID
                                sqlCmd.Parameters(1).Value = data.attributeID
                                sqlCmd.Parameters(2).Value = DBNull.Value
                                sqlCmd.Parameters(3).Value = data.value
    ...
    ''' <summary>
    ''' Class to hold an instance of a TypeAttribute
    ''' </summary>
    ''' <remarks></remarks>
