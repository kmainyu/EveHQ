...
Namespace CacheConversionClasses
    ...
    Public Class Types
        ...
        Friend Shared dataCollection As New List(Of EveType)
        Const tableName As String = "invTypes"
        Const tableColumns As Integer = 13
        Const deleteSQL As String = "DELETE FROM " & tableName & ";"
        Const createSQL As String = "DROP TABLE IF EXISTS " & tableName & "; CREATE TABLE " & tableName & "(typeID int NOT NULL, groupID int NULL, typeName text NULL, description text NULL, mass float NULL, volume float NULL, capacity float NULL, portionSize int NULL, raceID int NULL, basePrice float NULL, published boolean NULL, marketeGroupID int NULL, chanceOfDuplicating float NULL);"
        Const insertSQL As String = "INSERT INTO " & tableName & "(typeID, groupID, typeName, description, mass, volume, capacity, portionSize, raceID, basePrice, published, marketeGroupID, chanceOfDuplicating) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?);"
                ...
                Try
                    ...
                    Using sqlCmd As SQLiteCommand = connection.CreateCommand
                        Using sqlTrans As SQLiteTransaction = connection.BeginTransaction
                            ...
                            For Each data As EveType In dataCollection

                                ' Add the database values and execute the query
                                sqlCmd.Parameters(0).Value = data.typeID
                                sqlCmd.Parameters(1).Value = data.groupID
                                sqlCmd.Parameters(2).Value = data.typeName
                                sqlCmd.Parameters(3).Value = data.description
                                sqlCmd.Parameters(4).Value = data.mass
                                sqlCmd.Parameters(5).Value = data.volume
                                sqlCmd.Parameters(6).Value = data.capacity
                                sqlCmd.Parameters(7).Value = data.portionSize
                                sqlCmd.Parameters(8).Value = data.raceID
                                sqlCmd.Parameters(9).Value = data.basePrice
                                sqlCmd.Parameters(10).Value = data.published
                                sqlCmd.Parameters(11).Value = data.marketGroupID
                                sqlCmd.Parameters(12).Value = data.chanceOfDuplicating
    ...
    ''' <summary>
    ''' Class to hold an instance of a EveType
    ''' </summary>
    ''' <remarks></remarks>
    Public Class EveType
        ...
        Public Property chanceOfDuplicating As Double
    End Class
