...
Namespace CacheConversionClasses
    ...
    Public Class AttributeTypes
        ...
        Friend Shared dataCollection As New List(Of AttributeType)
        Const tableName As String = "dgmAttributeTypes"
        Const tableColumns As Integer = 12
        Const deleteSQL As String = "DELETE FROM " & tableName & ";"
        Const createSQL As String = "DROP TABLE IF EXISTS " & tableName & "; CREATE TABLE " & tableName & "(attributeID int NOT NULL,attributeName text NULL, description text NULL, iconID int NULL, defaultValue float NULL, published boolean NULL, displayName text NULL, unitID int NULL, stackable boolean NULL, highIsGood boolean NULL, categoryID int NULL, attributeGroup int NULL);"
        Const insertSQL As String = "INSERT INTO " & tableName & "(attributeID, attributeName, description, iconID, defaultValue, published, displayName, unitID, stackable, highIsGood, categoryID, attributeGroup) VALUES (?,?,?,?,?,?,?,?,?,?,?,?);"
                ...
                Try
                    ...
                    Using sqlCmd As SQLiteCommand = connection.CreateCommand
                        Using sqlTrans As SQLiteTransaction = connection.BeginTransaction
                            ...
                            For Each data As AttributeType In dataCollection

                                ' Add the database values and execute the query
                                sqlCmd.Parameters(0).Value = data.attributeID
                                sqlCmd.Parameters(1).Value = data.attributeName
                                sqlCmd.Parameters(2).Value = data.description
                                sqlCmd.Parameters(3).Value = data.iconID
                                sqlCmd.Parameters(4).Value = data.defaultValue
                                sqlCmd.Parameters(5).Value = data.published
                                sqlCmd.Parameters(6).Value = data.displayName
                                sqlCmd.Parameters(7).Value = data.unitID
                                sqlCmd.Parameters(8).Value = data.stackable
                                sqlCmd.Parameters(9).Value = data.highIsGood
                                sqlCmd.Parameters(10).Value = data.categoryID
                                sqlCmd.Parameters(11).Value = data.attributeGroup
    ...
    ''' <summary>
    ''' Class to hold an instance of a AttributeType
    ''' </summary>
    ''' <remarks></remarks>
    Public Class AttributeType
        ...
        Public Property attributeGroup As Integer
    End Class
