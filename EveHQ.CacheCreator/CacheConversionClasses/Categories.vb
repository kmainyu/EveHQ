...
Namespace CacheConversionClasses
    ...
    Public Class Categories
        ...
        Friend Shared dataCollection As New List(Of Category)
        Const tableName As String = "invCategories"
        Const tableColumns As Integer = 5
        Const deleteSQL As String = "DELETE FROM " & tableName & ";"
        Const createSQL As String = "DROP TABLE IF EXISTS " & tableName & "; CREATE TABLE " & tableName & "(categoryID int NOT NULL, categoryName text NULL, description text NULL, iconID int NULL, published boolean NULL);"
        Const insertSQL As String = "INSERT INTO " & tableName & "(categoryID, categoryName, description, iconID, published) VALUES (?,?,?,?,?);"
                ...
                Try
                    ...
                    Using sqlCmd As SQLiteCommand = connection.CreateCommand
                        Using sqlTrans As SQLiteTransaction = connection.BeginTransaction
                            ...
                            For Each data As Category In dataCollection

                                ' Add the database values and execute the query
                                sqlCmd.Parameters(0).Value = data.categoryID
                                sqlCmd.Parameters(1).Value = data.categoryName
                                sqlCmd.Parameters(2).Value = data.description
                                sqlCmd.Parameters(3).Value = data.iconID
                                sqlCmd.Parameters(4).Value = data.published
    ...
    ''' <summary>
    ''' Class to hold an instance of a Category
    ''' </summary>
    ''' <remarks></remarks>
