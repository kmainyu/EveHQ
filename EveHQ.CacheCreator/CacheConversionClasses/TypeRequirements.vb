...
Namespace CacheConversionClasses
    ...
    Public Class TypeRequirements
        Friend Shared dataCollection As New List(Of TypeRequirement)
        Const tableName As String = "ramTypeRequirements"
        Const tableColumns As Integer = 6
        Const deleteSQL As String = "DELETE FROM " & tableName & ";"
        Const createSQL As String = "DROP TABLE IF EXISTS " & tableName & "; CREATE TABLE " & tableName & "(typeID int NOT NULL, activityID int NOT NULL, requiredTypeID int NOT NULL, quantity int NULL, damagePerJob float NULL, recycle boolean NULL);"
        Const insertSQL As String = "INSERT INTO " & tableName & "(typeID, activityID, requiredTypeID, quantity, damagePerJob, recycle) VALUES (?,?,?,?,?,?);"
        ...
        Private Shared Sub ConvertCacheResultToClass(result As List(Of Object))
            ...
            For Each row As Object In result
                ...
                For Each keyValuePair As Dictionary(Of Object, Object) In CType(CType(row, Tuple(Of Object)).Item1, List(Of Object))
                    ...
                    Dim data As New TypeRequirement
                ...
                Try
                    ...
                    Using sqlCmd As SQLiteCommand = connection.CreateCommand
                        Using sqlTrans As SQLiteTransaction = connection.BeginTransaction
                            ...
                            For Each data As TypeRequirement In dataCollection

                                ' Add the database values and execute the query
                                sqlCmd.Parameters(0).Value = data.typeID
                                sqlCmd.Parameters(1).Value = data.activityID
                                sqlCmd.Parameters(2).Value = data.requiredTypeID
                                sqlCmd.Parameters(3).Value = data.quantity
                                sqlCmd.Parameters(4).Value = data.damagePerJob
                                sqlCmd.Parameters(5).Value = data.recycle
    ...
    ''' <summary>
    ''' Class to hold an instance of a TypeRequirement
    ''' </summary>
    ''' <remarks></remarks>
    Public Class TypeRequirement
        Public Property typeID As Integer
        Public Property activityID As Integer
        Public Property requiredTypeID As Integer
        Public Property quantity As Integer
        Public Property damagePerJob As Double
