...
#Region "Plug-in Startup Routines"
    ...
    Public Sub CheckForConqXMLFile()
        ...
        If My.Computer.FileSystem.FileExists(fileName) = True Then
            ...
            Call Me.ParseConquerableXML(stationXML)
        End If
    End Sub
    ...
    Private Function PopulateAssetNode(ByVal parentAsset As ContainerListViewItem, ByVal loc As XmlNode, ByVal assetOwner As String, ByVal location As String, ByVal selPilot As EveHQ.Core.Pilot) As Double
        ...
        For Each subLoc In subLocList
            Try
                ...
                Dim subFlagName As String = itemFlags(subFlagID).ToString
...
#Region "Outpost XML Retrieval and Parsing"
    ...
    End Sub
    Private Sub ParseConquerableXML(ByVal stationXML As XmlDocument)
        Dim locList As XmlNodeList
        Dim loc As XmlNode
        ...
        If locList.Count > 0 Then
            For Each loc In locList
                ...
                If CDbl(stationID) >= 61000001 And CDbl(stationID) <= 61999999 Then
                    ...
                    If stations.Contains(stationID) = False Then
                        ...
                        Dim system As SolarSystem = CType(stations(station.systemID), SolarSystem)
                        station.locationName &= " (" & system.Name & ", " & system.Region & ")"
                        stations.Add(station.locationID, station)
                    Else
                        Dim station As Location = CType(stations(stationID), Assets.Location)
                        station.systemID = (loc.Attributes.GetNamedItem("solarSystemID").Value)
                        Dim system As SolarSystem = CType(stations(station.systemID), SolarSystem)
                        station.locationName &= " (" & system.Name & ", " & system.Region & ")"
                    End If
...
#End Region
    ...
    Private Sub btnBuildRigs_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuildRigs.Click
        ...
        Dim strSQL As String = "SELECT TL2MaterialsForTypeWithActivity.typeID AS TL2MaterialsForTypeWithActivity_typeID, TL2MaterialsForTypeWithActivity.activity, TL2MaterialsForTypeWithActivity.requiredTypeID, TL2MaterialsForTypeWithActivity.quantity, TL2MaterialsForTypeWithActivity.damagePerJob, invTypes.typeID AS invTypes_typeID, invTypes.groupID, invTypes.published"
        strSQL &= " FROM invTypes INNER JOIN TL2MaterialsForTypeWithActivity ON invTypes.typeID = TL2MaterialsForTypeWithActivity.typeID"
        strSQL &= " WHERE (((TL2MaterialsForTypeWithActivity.activity)=1) AND ((invTypes.groupID)=787) AND ((invTypes.published)=True));"
