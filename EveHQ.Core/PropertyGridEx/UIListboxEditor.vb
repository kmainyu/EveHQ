    ...
    Public Class UIListboxEditor
        ...
        ITypeDescriptorContext) As UITypeEditorEditStyle
            If Not context Is Nothing AndAlso Not context.Instance Is Nothing Then
                Dim attribute As UIListboxIsDropDownResizable = CType(context.PropertyDescriptor.Attributes(GetType(UIListboxIsDropDownResizable)), UIListboxIsDropDownResizable)
        ...
        Public Overrides Function EditValue(ByVal context As System.ComponentModel.ITypeDescriptorContext, ByVal provider As System.IServiceProvider, ByVal value As Object) As Object
            ...
            oEditorService = CType(provider.GetService(GetType(IWindowsFormsEditorService)), IWindowsFormsEditorService)
            If oEditorService IsNot Nothing Then

                ' Get the Back reference to the Custom Property
                Dim oDescriptor As CustomProperty.CustomPropertyDescriptor = CType(context.PropertyDescriptor, CustomProperty.CustomPropertyDescriptor)
                Dim cp As CustomProperty = CType(oDescriptor.CustomProperty, CustomProperty)
                ...
                With context.PropertyDescriptor
                    datasource = CType(.Attributes(GetType(UIListboxDatasource)), UIListboxDatasource)
                    valuemember = CType(.Attributes(GetType(UIListboxValueMember)), UIListboxValueMember)
                    displaymember = CType(.Attributes(GetType(UIListboxDisplayMember)), UIListboxDisplayMember)
                End With
